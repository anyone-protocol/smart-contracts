...
---   · state read from `base.state`; a write's reply is the compute output ('OK', or the rewarded
...
describe('native staking-rewards — WASM-harness parity (Lua 5.3)', function()
  ...
  local function snapshot(base) return base.state.PreviousRound end
  ...
  describe('ACL enforcement', function()
    ...
    it('Update-Configuration: allows Admin Role', function()
      ...
      assert.are.equal('100', base.state.Configuration.TokensPerSecond)
    end)
  ...
  describe('Complete-Round', function()
    ...
    describe('uses default share for new operators', function()
      ...
      it('New operator uses default share (not persisted to Shares table)', function()
        ...
        round(base, 1000, REF)
        assert.is_nil(base.state.Shares[BOB])
      ...
      it('Existing operator retains their set share', function()
        ...
        round(base, 2000, REF)
        assert.are.equal(0.3, base.state.Shares[BOB])
      ...
      it('Multiple new operators all use default share (not persisted)', function()
        ...
        assert.is_nil(base.state.Shares[BOB])
        assert.is_nil(base.state.Shares[CHARLS])
  ...
  describe('Score rewards', function()
    ...
    it('Proportionally rewards hodlers based on their rating', function()
      ...
      assert.is_not_nil(base.state.Rewarded[ALICE])
      assert.is_nil(base.state.Rewarded[ALICE][BOB])
      ...
      assert.are.equal('1583', base.state.Rewarded[ALICE][BOB])
      assert.are.equal('3000', base.state.Rewarded[BOB][CHARLS])
      assert.are.equal('83', base.state.Rewarded[BOB][BOB])
      assert.are.equal('5333', base.state.Rewarded[CHARLS][CHARLS])   -- 4500 + 500 + 333
    end)
  ...
  describe('Claim-Rewards', function()
    ...
    it('Tracks Claimed, rewarded tokens', function()
      ...
      assert.are.equal('5333', base.state.Claimed[CHARLS][CHARLS])
      assert.are.equal('3000', json.decode(outData(claim(base, BOB)))[CHARLS])
      assert.are.equal('3000', base.state.Claimed[BOB][CHARLS])
      assert.are.equal('83', base.state.Claimed[BOB][BOB])
      ...
      assert.are.equal('253', base.state.Rewarded[BOB][BOB])
  ...
  describe('Update-Shares-Configuration', function()
    ...
    it('Updates only the provided field, leaving the others', function()
      ...
      local sh = base.state.Configuration.Shares
      assert.are.equal(0.2, sh.Min); assert.are.equal(0.9, sh.Max); assert.are.equal(0.5, sh.Default)
      updateShares(base, { Max = 0.8 })
      assert.are.equal(0.2, sh.Min); assert.are.equal(0.8, base.state.Configuration.Shares.Max)
      updateShares(base, { Default = 0.3 })
      assert.are.equal(0.3, base.state.Configuration.Shares.Default)
    end)
    ...
    describe('retroactive clamping of existing operator shares', function()
      ...
      it('Clamps existing shares when Min is raised', function()
        ...
        assert.are.equal(0.3, base.state.Shares[ALICE])
      end)
      it('Clamps existing shares when Max is lowered', function()
        ...
        assert.are.equal(0.5, base.state.Shares[ALICE])
      end)
  ...
  describe('Toggle-Feature-Shares', function()
    it('Enables and disables the shares feature', function()
      ...
      assert.is_true(base.state.Configuration.Shares.Enabled)
      assert.are.equal('OK', outData(toggleShares(base, false)))
      assert.is_false(base.state.Configuration.Shares.Enabled)
    end)
  ...
  describe('SetSharesEnabled', function()
    ...
    it('Can disable and re-enable set shares', function()
      ...
      assert.is_false(base.state.Configuration.Shares.SetSharesEnabled)
      updateShares(base, { SetSharesEnabled = true })
      assert.is_true(base.state.Configuration.Shares.SetSharesEnabled)
    end)
  ...
  describe('Share change delay', function()
    ...
    it('Allows configuring ChangeDelaySeconds', function()
      ...
      assert.are.equal(3600, base.state.Configuration.Shares.ChangeDelaySeconds)
    end)
    ...
    it('Allows ChangeDelaySeconds of 0 (immediate)', function()
      ...
      assert.are.equal(0, base.state.Configuration.Shares.ChangeDelaySeconds)
    end)
    it('Can update ChangeDelaySeconds together with other share config options', function()
      ...
      assert.are.equal(7200, base.state.Configuration.Shares.ChangeDelaySeconds)
    end)

    it('Applies share change immediately when ChangeDelaySeconds is 0', function()
      local base = b(0)
      setShare(base, BOB, 0.15, 1000)
      assert.are.equal(0.15, base.state.Shares[BOB])
      assert.is_nil(base.state.PendingShareChanges[BOB])
    end)
    it('Queues share change when ChangeDelaySeconds > 0', function()
      local base = b(3600)
      setShare(base, BOB, 0.2, 1000)
      assert.are.same({ Share = 0.2, RequestedTimestamp = 1000 }, base.state.PendingShareChanges[BOB])
      assert.is_nil(base.state.Shares[BOB])
    end)
    it('Replaces a pending change when Set-Share is called again', function()
      ...
      assert.are.same({ Share = 0.25, RequestedTimestamp = 2000 }, base.state.PendingShareChanges[BOB])
    end)
    it('Pending changes are visible in state', function()
      local base = b(3600)
      setShare(base, BOB, 0.15, 5000)
      assert.are.same({ Share = 0.15, RequestedTimestamp = 5000 }, base.state.PendingShareChanges[BOB])
    ...
    it('Does NOT apply a pending share change before the delay has passed', function()
      ...
      round(base, 2000, { [ALICE] = { [BOB] = stake('1000', 0.6) } })
      assert.is_nil(base.state.Shares[BOB])
      assert.are.same({ Share = 0.3, RequestedTimestamp = 1000 }, base.state.PendingShareChanges[BOB])
    end)
    it('Applies multiple pending changes when the delay has passed for each', function()
      ...
      assert.are.equal(0.1, base.state.Shares[BOB])
      assert.are.equal(0.2, base.state.Shares[CHARLS])
    end)
    it('Partial application — only applies changes whose delay has passed', function()
      ...
      assert.is_nil(base.state.Shares[CHARLS])
      assert.is_nil(base.state.PendingShareChanges[BOB])
      assert.are.same({ Share = 0.2, RequestedTimestamp = 1500 }, base.state.PendingShareChanges[CHARLS])
    end)
  end)
  ...
  describe('share-change delay — real millisecond timestamps', function()
    ...
    it('a 7-day delay does NOT elapse at the next hourly round (the legacy bug)', function()
      ...
      assert.is_nil(base.state.Shares[BOB])
      assert.are.same({ Share = 0.42, RequestedTimestamp = T0 + 1000 }, base.state.PendingShareChanges[BOB])
    end)
    ...
    it('a 7-day delay DOES elapse after 7 days', function()
      ...
      assert.are.equal(0.42, base.state.Shares[BOB])
      assert.is_nil(base.state.PendingShareChanges[BOB])
    end)
    ...
    it('an hour-long delay elapses on the hour, not 3.6 seconds later', function()
      ...
      assert.are.equal(0.2, base.state.Shares[BOB])
    end)
  end)
  ...
  describe('Runtime safety (D8 axes)', function()
    ...
    it('Reverts state atomically when a handler errors', function()
      ...
      local before = native.deepcopy(base.state.Rewarded)
      completeRound(base, 999999)
      assert.is_true(has(outData(base), 'No pending round for'))
      assert.are.same(before, base.state.Rewarded)
    end)
    it('A17: PendingRounds is keyed by the string timestamp, never the integer', function()
      ...
      assert.is_not_nil(base.state.PendingRounds['1783064040855'])
      assert.is_nil(base.state.PendingRounds[1783064040855])
    end)
  end)
end)
