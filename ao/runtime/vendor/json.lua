...
local function encode_number(val)
    ...
    -- LUERL-PATCH (Memetic Block): encode integers exactly. %.14g corrupts integers
    -- with >= 15 significant digits (e.g. token amounts) into lossy scientific notation.
...
local function parse_number(str, i)
    ...
    local n
    -- LUERL-PATCH (Memetic Block): luerl's tonumber FLOATS integer literals; digit-fold
    -- integer literals to keep integer subtype (exact, arbitrary precision under luerl).
    -- Floats (containing . e or E) go through tonumber unchanged.
    if s:match('^%-?%d+$') then
        local a, neg = s, false
        if a:sub(1, 1) == '-' then neg, a = true, a:sub(2) end
        n = 0
        for k = 1, #a do n = n * 10 + (a:byte(k) - 48) end
        if neg then n = -n end
    else
        n = tonumber(s)
    end
