...
let consulToken = process.env.CONSUL_TOKEN,
  ...
  previousDistributionTrackingLimit =
    process.env.DISTRIBUTION_PREV_TRACKING_LIMIT
...
async function main() {
  if (!previousDistributionTrackingLimit) {
    throw new Error('DISTRIBUTION_PREV_TRACKING_LIMIT is not set!')
  }
  ...
  const input: SetPreviousDistributionTrackingLimit = {
    function: 'setPreviousDistributionTrackingLimit',
    limit: Number.parseInt(previousDistributionTrackingLimit)
  }
  ...
  await contract
    ...
    .writeInteraction<SetPreviousDistributionTrackingLimit>(input)
}
